...
function solution(hp) {
  ...
  if(hp / generalAnt > 0) {
    ...
    hp = hp - (generalAnt * Math.floor(hp / generalAnt));
  }

  if(hp / soldierAnt > 0) {
    answer += Math.floor(hp / soldierAnt);
    hp = hp - (soldierAnt * Math.floor(hp / soldierAnt));
  }

  if(hp / normalAnt > 0) {
    answer += Math.floor(hp / normalAnt);
    hp = hp - (normalAnt * Math.floor(hp / normalAnt));
  }
